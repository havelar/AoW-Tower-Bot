...
class TowerConfig:
    ...
    @classmethod
    def getPoints(self):
        self.curStatus['text'] = 'Click on the upper left corner\nand on the right bottom.'
        self.curStatus.update()
        self.P1, self.P2 = CL.getClicks(nr = 2)
        self.curStatus['text'] = 'Selected points:\nP1:{0}\nP2:{1}'.format(self.P1,self.P2)

    @classmethod
    def getGrindPath(self):
        self.curStatus['text'] = 'Select as many maps you\nwant with middle button.\nPress right button to stop.'
        self.curStatus.update()
        self.grindPath = CL.getClicks(10)
    ...
    def startBot(self):
        self.config.destroy()
        self.TC = TowerLogWindow(self.P1, self.P2, self.grindModeStr, self.grindAmount.get(),self.grindGCReward.get(), self.grindPath, self.grindTimes.get())
    ...
    def configWindow(self):
        ...
        om1=OptionMenu(self.config, self.grindMode, *self.modeList, command=self.setGrindMode)
        ...
        grindGCReward_checkbox = Checkbutton(self.config, variable=self.grindGCReward,  text='Use GC to get boss rewards.')
        grindGCReward_checkbox.place(x=25, y=115)

        self.getLoc = Button(self.config, width=20, height=1, text='Select screen margin.', command=TowerConfig.getPoints)
        self.getLoc.place(x=25, y = 165)

        getPath = Button(self.config, width=20, height=1, text='Select Levels.', command=TowerConfig.getGrindPath)
        ...
        self.startButton = Button(self.config, text='Start', width=20, height=1, command=TowerConfig.startBot)
